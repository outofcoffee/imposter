...
class S3FileDownloader private constructor() {
    ...
    fun downloadAllFiles(s3BaseUrl: String, destDir: File) {
        ...
        val files: List<String> = listFiles(s3BaseUrl)
        if (files.isEmpty()) {
     ...
     */
    private fun determineBucketName(s3Url: String) =
        s3Url.substring(5, s3Url.indexOf("/", 5))

    companion object {
        const val ENV_S3_API_ENDPOINT = "IMPOSTER_S3_API_ENDPOINT"
        const val SYS_PROP_S3_API_ENDPOINT = "imposter.s3.api.endpoint"
        private val LOGGER = LoggerFactory.getLogger(S3FileDownloader::class.java)
