...
object HttpUtil {
    ...
    const val DEFAULT_HTTPS_LISTEN_PORT = 8443
    ...
    fun readAcceptedContentTypes(acceptHeader: String?): List<String> {
        ...
        val accepts = acceptEntries.map { weightAndContentType: String ->
            WeightedAcceptEntry.parse(weightAndContentType)
        }

        // sort with highest at the top (descending order)
        return accepts
            .sortedWith { o1, o2 -> o2.weight.compareTo(o1.weight) }
...
"version":"${MetaUtil.readVersion()}"
    ...
    private class WeightedAcceptEntry(
        ...
        companion object {
            fun parse(weightAndContentType: String): WeightedAcceptEntry {
                // split to obtain weight
                val sEntry = weightAndContentType.split(";").toTypedArray()

                return WeightedAcceptEntry(
                    contentType = sEntry[0],
                    weight = if (sEntry.size > 1 && sEntry[1].lowercase(Locale.getDefault()).startsWith("q=")) {
                        java.lang.Float.valueOf(sEntry[1].substring(2))
                    } else {
                        0f
                    }
                )
