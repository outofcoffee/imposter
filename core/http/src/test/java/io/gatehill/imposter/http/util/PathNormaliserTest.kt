...
/**
 * Tests for [PathNormaliser].
 *
 * @author Pete Cornish
 */
class PathNormaliserTest {
    @Test
    fun `should convert path to Vertx format`() {
        val normalisedParams = mutableMapOf<String, String>()
        val result = PathNormaliser.normalisePath(normalisedParams,"/{pathParam}/notParam")
    ...
    fun `should handle param and plain string`() {
        val normalisedParams = mutableMapOf<String, String>()
        val result = PathNormaliser.normalisePath(normalisedParams,"/{firstParam}.notParam")
    ...
    fun `should normalise path`() {
        val normalisedParams = mutableMapOf<String, String>()
        val result = PathNormaliser.normalisePath(normalisedParams, "/{firstParam}/{second-param}/notParam")
    ...
    fun `should get normalised param name`() {
        ...
        assertEquals("abcdef1234567890", PathNormaliser.getNormalisedParamName(normalisedParams, "kebab-param"))
        assertEquals("normalParam", PathNormaliser.getNormalisedParamName(normalisedParams, "normalParam"))
    }
    ...
    fun `should denormalise params`() {
        ...
        val result = PathNormaliser.denormaliseParams(normalisedParams, vertxParams)
        assertEquals("param value", result["kebab-param"])
        assertEquals("application/json", result["content-type"])
    }

    @Test
    fun `should return null if path is null`() {
        val normalisedParams = mutableMapOf<String, String>()
        val result = PathNormaliser.normalisePath(normalisedParams, null)
